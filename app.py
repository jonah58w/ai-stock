...
# ✅ 逐路診斷（哪一路失敗、為什麼、preview）
# ✅ 指標共振：MA/RSI/MACD/KD/布林/ATR/量能
# ✅ 布林通道 + 均線 + 買賣點標記（Matplotlib）
# ✅ Retry + 缺套件提示（側邊欄）
#
# 注意：
# - Streamlit Cloud 若缺套件：請在 requirements.txt 加入 FinMind / yfinance / pandas_datareader / matplotlib
...
def _as_str_ticker(x) -> str:
    """防止 tuple/list 被誤傳進來造成 yfinance .lower() 爆炸。"""
...
def _retry(fn, times=3, sleep=0.7):
    last = None
...
def _preview_text(text: str, maxlen=160) -> str:
    t = (text or "").replace("\n", " ").strip()
...
def _looks_like_html(text: str) -> bool:
    ...
    if "<html" in t or "<!doctype html" in t:
        return True
    # WAF pages often include these:
    waf_tokens = ["cloudflare", "attention required", "access denied", "akamai", "waf"]
    return any(tok in t for tok in waf_tokens)
...
def fetch_yfinance(code: str, period_days: int) -> Tuple[Optional[pd.DataFrame], FetchAttempt]:
    ...
    try:
        for tk in tickers:
            ...
            def _do():
                return yf.download(tk, period=f"{max(30, period_days)}d", interval="1d", auto_adjust=False, progress=False)
            ...
            # yfinance returns index as DatetimeIndex, columns may be multiindex
            df = raw.copy()
...
def fetch_stooq(code: str, period_days: int) -> Tuple[Optional[pd.DataFrame], FetchAttempt]:
    code = _as_str_ticker(code)
    # stooq tickers for TW not always consistent; this is best-effort fallback
    # We'll try stooq via pandas_datareader if available, else HTTP CSV.
    try:
        if DEPENDENCIES.get("pandas_datareader", False) and pdr is not None:
            # Many TW tickers are not supported; still try
            ticker = code
            def _do():
                # stooq sometimes expects lower-case; safe to lower
                return pdr.DataReader(ticker.lower(), "stooq")
        ...
        # HTTP fallback (very best-effort)
...
def fetch_ohlcv_multi(code: str, months_back: int, csv_upload=None) -> Tuple[Optional[pd.DataFrame], str, List[FetchAttempt]]:
    ...
    # 2026 建議順序：FinMind → yfinance → Stooq → CSV
...
def stochastic_kd(high: pd.Series, low: pd.Series, close: pd.Series, k=14, d=3) -> Tuple[pd.Series, pd.Series]:
    ll = low.rolling(k).min()
    hh = high.rolling(k).max()
    k_line = 100 * (close - ll) / (hh - ll).replace(0, np.nan)
    d_line = k_line.rolling(d).mean()
    return k_line.fillna(method="bfill"), d_line.fillna(method="bfill")
...
def bollinger(close: pd.Series, window=20, n_std=2.0) -> Tuple[pd.Series, pd.Series, pd.Series]:
...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["VOL_Z20"] = (vol - vol.rolling(20).mean()) / (vol.rolling(20).std().replace(0, np.nan))
    df["VOL_Z20"] = df["VOL_Z20"].replace([np.inf, -np.inf], np.nan).fillna(0)
...
def resonance_score(latest: pd.Series) -> Tuple[int, Dict[str, Any]]:
    ...
    if not np.isnan(rsi14):
        if rsi14 >= 70:
            score -= 4  # overbought risk
            details["rsi_zone"] = "overbought"
        elif rsi14 <= 30:
            score += 6  # oversold mean-reversion opportunity
...
def determine_action(df: pd.DataFrame, strategy: str, params: Dict[str, float]) -> Tuple[str, str]:
    ...
    # --- Pullback strategy: buy near support / lower band with reversal signs
    if strategy == "pullback":
        # near lower band or near ema20 with oversold / turning
        ...
        # sell / reduce near upper band with overbought and weakening
    ...
    # --- Breakout strategy: chase within acceptable distance after breakout confirmation
        ...
        # breakout fail / stop: drop back under EMA20 or under trigger
...
def estimate_zones(df: pd.DataFrame, max_buy_distance: float) -> Dict[str, Any]:
    """
    Provide professional buy/sell zones:
      - near-term actionable buy zone (<= max_buy_distance)
      - deep pullback buy zone
      - near-term sell/TP zone
    """
    latest = df.iloc[-1]
    close = float(latest["Close"])
    atr14 = float(latest.get("ATR14", np.nan))
    ...
    # Support/resistance from rolling
    ...
    # Actionable buy zone: between min(ema20, bb_mid) and bb_lo (if sensible), clamped near close by max_buy_distance
    base_lo = min(bb_lo if not np.isnan(bb_lo) else close - 1.2 * atr14,
                  ema20 if not np.isnan(ema20) else close - 1.0 * atr14,
                  bb_mid if not np.isnan(bb_mid) else close - 0.8 * atr14)
    base_hi = min(ema20 if not np.isnan(ema20) else close - 0.5 * atr14,
    ...
    # clamp to "not too far"
    min_price_allowed = close * (1 - max_buy_distance)
    z1_lo = max(z1_lo, min_price_allowed)
    z1_hi = max(min(z1_hi, close), z1_lo)

    # deep pullback buy zone: rolling support area +/- ATR
    z2_center = float(sup)
    z2_lo = max(0.01, z2_center - 0.8 * atr14)
    z2_hi = z2_center + 0.5 * atr14

    # sell/TP zone: near resistance and upper band
    tp_base = max(res * 0.98, bb_up * 0.985 if not np.isnan(bb_up) else res * 0.98)
    z3_lo = tp_base
    z3_hi = tp_base + 0.8 * atr14

    def _dist_str(target: float) -> Tuple[float, str]:
        p = (target - close) / close
        # professional phrasing:
        # negative => pullback needed; positive => upside to target
        return float(p), _human_pct(p)

    d1, d1s = _dist_str((z1_lo + z1_hi) / 2)
    d2, d2s = _dist_str((z2_lo + z2_hi) / 2)
    d3, d3s = _dist_str((z3_lo + z3_hi) / 2)

    return {
        "close": close,
        "action_buy_zone": (float(z1_lo), float(z1_hi), d1, d1s),
        "deep_buy_zone": (float(z2_lo), float(z2_hi), d2, d2s),
        "sell_zone": (float(z3_lo), float(z3_hi), d3, d3s),
        "support": float(sup),
...
def plot_bollinger(df: pd.DataFrame, title: str, marks: Optional[List[Tuple[pd.Timestamp, float, str]]] = None):
    """
    Bollinger + Close + SMA20 + EMA20 + markers.
    marks: list of (date, price, label)
    """
    x = df["Date"]
    c = df["Close"]
    bb_up = df.get("BB_UP")
    ...
    if bb_up is not None and bb_mid is not None and bb_lo is not None:
        ax.plot(x, bb_up, label="BB Upper")
        ax.plot(x, bb_mid, label="BB Mid")
        ax.plot(x, bb_lo, label="BB Lower")
        ax.fill_between(x, bb_lo, bb_up, alpha=0.10)
    if sma20 is not None:
        ax.plot(x, sma20, label="SMA20")
    if ema20 is not None:
        ax.plot(x, ema20, label="EMA20")
...
def scan_top10(codes: List[str], months_back: int, max_buy_distance: float, strategy: str, params: Dict[str, float]) -> pd.DataFrame:
    ...
    for code in codes:
        df, src, attempts = fetch_ohlcv_multi(code, months_back, csv_upload=None)
        if df is None or len(df) < 60:
            continue
        df = add_indicators(df)
        score, details = resonance_score(df.iloc[-1])
        action, reason = determine_action(df, strategy, params)
        zones = estimate_zones(df, max_buy_distance)

        close = float(df.iloc[-1]["Close"])
        buy_lo, buy_hi, d1, d1s = zones["action_buy_zone"]
    ...
    # Sort by score then BUY first
    out["__buy_rank"] = out["當下判斷"].map({"BUY": 0, "WATCH": 1, "SELL": 2}).fillna(3)
    out = out.sort_values(["__buy_rank", "AI分數"], ascending=[True, False]).drop(columns="__buy_rank")
    out = out.drop_duplicates(subset=["股票"], keep="first")  # avoid duplicates
...
with st.sidebar:
    ...
    # distance constraint (avoid unrealistic buy point)
    max_buy_distance = st.slider("可操作買點最大距離（避免買點離現實太遠）", 0.02, 0.20, 0.12, 0.01, key="max_buy_distance")
    ...
    if strategy == "breakout":
        ...
        params["stop_atr"] = st.slider("失效止損距離（ATR 倍數）", 0.5, 4.0, 1.6, 0.1)
        params["rr"] = st.slider("目標風險報酬（RR）", 1.0, 5.0, 2.2, 0.1)
    ...
    if not DEPENDENCIES["finmind"]:
        st.info("建議安裝 FinMind：requirements.txt 加入 FinMind>=1.0.0")
    else:
        token_hint = "✅ 已偵測 FinMind 套件"
    ...
    if st.button("網路測試（建議先按一次）"):
        ...
        # yfinance test (only if installed)
...
if mode == "單一股票分析":
    ...
    buy_lo, buy_hi, d1, d1s = zones["action_buy_zone"]
    deep_lo, deep_hi, d2, d2s = zones["deep_buy_zone"]
    sell_lo, sell_hi, d3, d3s = zones["sell_zone"]
    ...
    marks = []
    # Mark action points (simple: last signal markers)
    # Mark where Close crosses above EMA20 (bull) / below EMA20 (bear)
...
# -----------------------------
    ...
    if st.button("開始掃描 Top10"):
        ...
        if topdf is None or topdf.empty:
            ...
            st.caption("提示：Top10 顯示『當下判斷（BUY/SELL/WATCH）』與『可操作買區』，可直接用來做明日測試。")
