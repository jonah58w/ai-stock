# ==============================================================
# AI 股票量化分析系統 V12 PRO（完整可直接覆蓋版）
# 單檔分析 + 全台股 Top10 掃描 + 價值分析 + 技術分析 + AI 評分
#
# 設計原則
# - 優先穩定可跑，不做危險簡化
# - 保留原有核心功能：
#   1) 單一股票分析
#   2) Top 10 掃描
#   3) 技術分析：K線 / 布林通道 / SMA20/50/200 / MACD / KD / RSI / ATR
#   4) 價值分析：殖利率 / PE / PB / EPS / ROE / 現金股利 / 合理價估值
#   5) AI 綜合評分 / 建議 / 買賣點 / 停損 / 風險報酬比
#   6) FinMind 優先 + Yahoo fallback
#   7) FinMind 沒 token 時，掃描功能仍可用備援股票池
# - 繁體中文介面
#
# requirements.txt 建議：
# streamlit
...
def get_fallback_universe() -> pd.DataFrame:
    ...
    rows = []
    for sid, name, typ, ind in fallback:
        rows.append(
            {
                "stock_id": sid,
                "stock_name": name,
...
def load_fundamental(stock_id: str, market_type: Optional[str] = None, token: Optional[str] = None) -> Dict[str, object]:
    ...
    # 3) Yahoo 補值：PE / PB / EPS / ROE / dividend / yield
    for ysym in yahoo_symbol_candidates(stock_id, market_type):
        try:
            tk = yf.Ticker(ysym)
            info = tk.info or {}
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("Top10 會優先掃描 FinMind 全台股清單；若抓不到，系統仍會使用備援股票池繼續運作。")
...
# ============================================================== 
    ...
    if start_scan:
        try:
            ...
            if universe.empty:
                st.warning("FinMind 全台股清單抓不到，系統將自動改用備援股票池繼續掃描。")
